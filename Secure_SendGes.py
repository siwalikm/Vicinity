...
import sys, time
from socket import *
xx = socket(AF_INET, SOCK_DGRAM)
xx.bind(('', 0))
xx.setsockopt(SOL_SOCKET, SO_BROADCAST, 1)
import random
def randstring(length=5):
    valid_letters='qwertyuiopasdfghjklzxcvbnm'
    return ''.join((random.choice(valid_letters) for i in xrange(length)))
otp = randstring()
print 'your otp for this transfer is '+otp
xx.sendto(otp, ('<broadcast>', OTP_PORT)) 
...
while True:
    ...
    while(cam):
        ret, img = cap.read()
        cv2.rectangle(img,(250,250),(50,50),(0,255,0),1) ###Deleting this is causing an error,dont know why!
        ...
        _, contours, hierarchy = cv2.findContours(thresh1.copy(),cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
        ...
        cv2.rectangle(crop_img,(x,y),(x+w,y+h),(0,0,255),0) #Red box in the img window from the countours
